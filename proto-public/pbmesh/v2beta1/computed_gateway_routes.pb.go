// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: MPL-2.0

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.31.0
// 	protoc        (unknown)
// source: pbmesh/v2beta1/computed_gateway_routes.proto

package meshv2beta1

import (
	pbresource "github.com/hashicorp/consul/proto-public/pbresource"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// This is a Resource type.
type ComputedGatewayConfiguration struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// ListenerConfigs is the map of listener ports to the ComputedPortRoutes for
	// those ports.
	//
	// The port identifier key here is always normalized to the target (workload)
	// port name regardless of whether a virtual or target port identifier was
	// provided in input config.
	ListenerConfigs map[string]*ComputedGatewayListener `protobuf:"bytes,1,rep,name=listener_configs,json=listenerConfigs,proto3" json:"listener_configs,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
	// BoundReferences is a slice of mixed type references of resources that were
	// involved in the formulation of this resource.
	BoundReferences []*pbresource.Reference `protobuf:"bytes,2,rep,name=bound_references,json=boundReferences,proto3" json:"bound_references,omitempty"`
}

func (x *ComputedGatewayConfiguration) Reset() {
	*x = ComputedGatewayConfiguration{}
	if protoimpl.UnsafeEnabled {
		mi := &file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ComputedGatewayConfiguration) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComputedGatewayConfiguration) ProtoMessage() {}

func (x *ComputedGatewayConfiguration) ProtoReflect() protoreflect.Message {
	mi := &file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComputedGatewayConfiguration.ProtoReflect.Descriptor instead.
func (*ComputedGatewayConfiguration) Descriptor() ([]byte, []int) {
	return file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescGZIP(), []int{0}
}

func (x *ComputedGatewayConfiguration) GetListenerConfigs() map[string]*ComputedGatewayListener {
	if x != nil {
		return x.ListenerConfigs
	}
	return nil
}

func (x *ComputedGatewayConfiguration) GetBoundReferences() []*pbresource.Reference {
	if x != nil {
		return x.BoundReferences
	}
	return nil
}

type ComputedGatewayListener struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// HostnameConfigs is the map of the ComputedPortRoutes for
	// those ports.
	//
	// The port identifier key here is always normalized to the target (workload)
	// port name regardless of whether a virtual or target port identifier was
	// provided in input config.
	HostnameConfigs map[string]*ComputedHostnameRoutes `protobuf:"bytes,1,rep,name=hostname_configs,json=hostnameConfigs,proto3" json:"hostname_configs,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
	Port            uint32                             `protobuf:"varint,2,opt,name=port,proto3" json:"port,omitempty"`
}

func (x *ComputedGatewayListener) Reset() {
	*x = ComputedGatewayListener{}
	if protoimpl.UnsafeEnabled {
		mi := &file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ComputedGatewayListener) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComputedGatewayListener) ProtoMessage() {}

func (x *ComputedGatewayListener) ProtoReflect() protoreflect.Message {
	mi := &file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComputedGatewayListener.ProtoReflect.Descriptor instead.
func (*ComputedGatewayListener) Descriptor() ([]byte, []int) {
	return file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescGZIP(), []int{1}
}

func (x *ComputedGatewayListener) GetHostnameConfigs() map[string]*ComputedHostnameRoutes {
	if x != nil {
		return x.HostnameConfigs
	}
	return nil
}

func (x *ComputedGatewayListener) GetPort() uint32 {
	if x != nil {
		return x.Port
	}
	return 0
}

type ComputedHostnameRoutes struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// HostnameConfigs is the map of the ComputedPortRoutes for
	// those ports.
	//
	// The port identifier key here is always normalized to the target (workload)
	// port name regardless of whether a virtual or target port identifier was
	// provided in input config.
	Routes *ComputedPortRoutes         `protobuf:"bytes,1,opt,name=routes,proto3" json:"routes,omitempty"`
	Tls    *APIGatewayTLSConfiguration `protobuf:"bytes,2,opt,name=tls,proto3" json:"tls,omitempty"`
}

func (x *ComputedHostnameRoutes) Reset() {
	*x = ComputedHostnameRoutes{}
	if protoimpl.UnsafeEnabled {
		mi := &file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ComputedHostnameRoutes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComputedHostnameRoutes) ProtoMessage() {}

func (x *ComputedHostnameRoutes) ProtoReflect() protoreflect.Message {
	mi := &file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComputedHostnameRoutes.ProtoReflect.Descriptor instead.
func (*ComputedHostnameRoutes) Descriptor() ([]byte, []int) {
	return file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescGZIP(), []int{2}
}

func (x *ComputedHostnameRoutes) GetRoutes() *ComputedPortRoutes {
	if x != nil {
		return x.Routes
	}
	return nil
}

func (x *ComputedHostnameRoutes) GetTls() *APIGatewayTLSConfiguration {
	if x != nil {
		return x.Tls
	}
	return nil
}

var File_pbmesh_v2beta1_computed_gateway_routes_proto protoreflect.FileDescriptor

var file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDesc = []byte{
	,
}

var (
	file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescOnce sync.Once
	file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescData = file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDesc
)

func file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescGZIP() []byte {
	file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescOnce.Do(func() {
		file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescData = protoimpl.X.CompressGZIP(file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescData)
	})
	return file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDescData
}

var file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes = make([]protoimpl.MessageInfo, 5)
var file_pbmesh_v2beta1_computed_gateway_routes_proto_goTypes = []interface{}{
	(*ComputedGatewayConfiguration)(nil), // 0: hashicorp.consul.mesh.v2beta1.ComputedGatewayConfiguration
	(*ComputedGatewayListener)(nil),      // 1: hashicorp.consul.mesh.v2beta1.ComputedGatewayListener
	(*ComputedHostnameRoutes)(nil),       // 2: hashicorp.consul.mesh.v2beta1.ComputedHostnameRoutes
	nil,                                  // 3: hashicorp.consul.mesh.v2beta1.ComputedGatewayConfiguration.ListenerConfigsEntry
	nil,                                  // 4: hashicorp.consul.mesh.v2beta1.ComputedGatewayListener.HostnameConfigsEntry
	(*pbresource.Reference)(nil),         // 5: hashicorp.consul.resource.Reference
	(*ComputedPortRoutes)(nil),           // 6: hashicorp.consul.mesh.v2beta1.ComputedPortRoutes
	(*APIGatewayTLSConfiguration)(nil),   // 7: hashicorp.consul.mesh.v2beta1.APIGatewayTLSConfiguration
}
var file_pbmesh_v2beta1_computed_gateway_routes_proto_depIdxs = []int32{
	3, // 0: hashicorp.consul.mesh.v2beta1.ComputedGatewayConfiguration.listener_configs:type_name -> hashicorp.consul.mesh.v2beta1.ComputedGatewayConfiguration.ListenerConfigsEntry
	5, // 1: hashicorp.consul.mesh.v2beta1.ComputedGatewayConfiguration.bound_references:type_name -> hashicorp.consul.resource.Reference
	4, // 2: hashicorp.consul.mesh.v2beta1.ComputedGatewayListener.hostname_configs:type_name -> hashicorp.consul.mesh.v2beta1.ComputedGatewayListener.HostnameConfigsEntry
	6, // 3: hashicorp.consul.mesh.v2beta1.ComputedHostnameRoutes.routes:type_name -> hashicorp.consul.mesh.v2beta1.ComputedPortRoutes
	7, // 4: hashicorp.consul.mesh.v2beta1.ComputedHostnameRoutes.tls:type_name -> hashicorp.consul.mesh.v2beta1.APIGatewayTLSConfiguration
	1, // 5: hashicorp.consul.mesh.v2beta1.ComputedGatewayConfiguration.ListenerConfigsEntry.value:type_name -> hashicorp.consul.mesh.v2beta1.ComputedGatewayListener
	2, // 6: hashicorp.consul.mesh.v2beta1.ComputedGatewayListener.HostnameConfigsEntry.value:type_name -> hashicorp.consul.mesh.v2beta1.ComputedHostnameRoutes
	7, // [7:7] is the sub-list for method output_type
	7, // [7:7] is the sub-list for method input_type
	7, // [7:7] is the sub-list for extension type_name
	7, // [7:7] is the sub-list for extension extendee
	0, // [0:7] is the sub-list for field type_name
}

func init() { file_pbmesh_v2beta1_computed_gateway_routes_proto_init() }
func file_pbmesh_v2beta1_computed_gateway_routes_proto_init() {
	if File_pbmesh_v2beta1_computed_gateway_routes_proto != nil {
		return
	}
	file_pbmesh_v2beta1_api_gateway_proto_init()
	file_pbmesh_v2beta1_computed_routes_proto_init()
	if !protoimpl.UnsafeEnabled {
		file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ComputedGatewayConfiguration); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ComputedGatewayListener); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ComputedHostnameRoutes); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   5,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_pbmesh_v2beta1_computed_gateway_routes_proto_goTypes,
		DependencyIndexes: file_pbmesh_v2beta1_computed_gateway_routes_proto_depIdxs,
		MessageInfos:      file_pbmesh_v2beta1_computed_gateway_routes_proto_msgTypes,
	}.Build()
	File_pbmesh_v2beta1_computed_gateway_routes_proto = out.File
	file_pbmesh_v2beta1_computed_gateway_routes_proto_rawDesc = nil
	file_pbmesh_v2beta1_computed_gateway_routes_proto_goTypes = nil
	file_pbmesh_v2beta1_computed_gateway_routes_proto_depIdxs = nil
}
