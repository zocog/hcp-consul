...
func (s *ResourceGenerator) makeAPIGatewayListeners(address string, cfgSnap *proxycfg.ConfigSnapshot) ([]proto.Message, error) {
	...
	for listenerRef, upstreams := range cfgSnap.APIGateway.Upstreams {
		...
		tlsContext, err := makeDownstreamTLSContext(cfgSnap, *listenerCfg)
		if err != nil {
			return nil, err
		}
...
func makeDownstreamTLSContextFromSnapshotAPIGatewayListenerConfig(cfgSnap *proxycfg.ConfigSnapshot, protocol string, gatewayTLSConfig structs.GatewayTLSConfig) (*envoy_tls_v3.DownstreamTlsContext, error) {
	var downstreamContext *envoy_tls_v3.DownstreamTlsContext

	tlsContext, err := makeCommonTLSContextFromSnapshotAPIGatewayListenerConfig(cfgSnap, gatewayTLSCfg)
	if err != nil {
		return nil, err
	}
...
func makeCommonTLSContextFromSnapshotListenerConfig(cfgSnap *proxycfg.ConfigSnapshot, gatewayTLSCfg structs.GatewayTLSConfig) (*envoy_tls_v3.CommonTlsContext, error) {
