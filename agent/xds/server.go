...
func (s *Server) authorize(_ context.Context, proxySnapshot proxysnapshot.ProxySnapshot) error {
	if proxySnapshot == nil {
		return status.Errorf(codes.Unauthenticated, "unauthenticated: no config snapshot")
	}

	// TODO(jm): uncomment this
	//authz, err := s.authenticate(ctx)
	//if err != nil {
	//	return err
	//}

	return nil //proxySnapshot.Authorize(authz)
}
