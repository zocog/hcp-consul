...
func (r *agentCacheRootsReader) Get() (*structs.IndexedCARoots, error) {
	...
	rawRoots, _, err := r.cache.Get(context.Background(), connectCARootName, &structs.DCSpecificRequest{
...
func (r *agentCacheRootsReader) Notify(ctx context.Context, correlationID string, ch chan<- cacheshim.UpdateEvent) error {
	return r.cache.Notify(ctx, connectCARootName, &structs.DCSpecificRequest{
		Datacenter: r.datacenter,
	}, correlationID, ch)
}
