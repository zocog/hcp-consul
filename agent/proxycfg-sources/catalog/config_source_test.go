...
func TestConfigSource_Success(t *testing.T) {
	...
	session1TermCh := make(proxysnapshot.SessionTerminatedChan)
	session1.On("Terminated").Return(session1TermCh)
	session1.On("End").Return()

	session2 := newMockSession(t)
	session2TermCh := make(proxysnapshot.SessionTerminatedChan)
...
func (nullSessionLimiter) BeginSession() (proxysnapshot.Session, error) {
...
func (nullSession) Terminated() proxysnapshot.SessionTerminatedChan { return nil }
...
func (m *mockSession) Terminated() proxysnapshot.SessionTerminatedChan {
	return m.Called().Get(0).(proxysnapshot.SessionTerminatedChan)
}
