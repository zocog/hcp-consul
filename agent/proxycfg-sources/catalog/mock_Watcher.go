...
func (_m *MockWatcher) Watch(proxyID *pbresource.ID, nodeName string, token string) (<-chan proxysnapshot.ProxySnapshot, proxysnapshot.SessionTerminatedChan, proxysnapshot.CancelFunc, error) {
	ret := _m.Called(proxyID, nodeName, token)

	var r0 <-chan proxysnapshot.ProxySnapshot
	var r1 proxysnapshot.SessionTerminatedChan
	var r2 proxysnapshot.CancelFunc
	var r3 error
	if rf, ok := ret.Get(0).(func(*pbresource.ID, string, string) (<-chan proxysnapshot.ProxySnapshot, proxysnapshot.SessionTerminatedChan, proxysnapshot.CancelFunc, error)); ok {
	...
	if rf, ok := ret.Get(1).(func(*pbresource.ID, string, string) proxysnapshot.SessionTerminatedChan); ok {
		r1 = rf(proxyID, nodeName, token)
	} else {
		if ret.Get(1) != nil {
			r1 = ret.Get(1).(proxysnapshot.SessionTerminatedChan)
		}
	}
