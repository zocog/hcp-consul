...
func NewOTELReader(client client.MetricsClient, exportInterval time.Duration) otelsdk.Reader {
...
func NewOTELSink(client client.MetricsClient, logger hclog.Logger) (*OTELSink, error) {
	return NewOTELSinkWithReader(NewOTELReader(client, 10*time.Second), logger)
}

func NewOTELSinkWithReader(reader otelsdk.Reader, logger hclog.Logger) (*OTELSink, error) {
	// Setup OTEL Metrics SDK to aggregate, convert and export metrics periodically.
	res := resource.NewSchemaless()
	meterProvider := otelsdk.NewMeterProvider(otelsdk.WithResource(res), otelsdk.WithReader(reader))
