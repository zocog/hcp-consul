...
type Registry interface {
	...
	Translate(from, to *pbresource.Type) (TranslationFunction, bool)
}
...
type TranslationFunction func(proto.Message) (proto.Message, error)
...
func (r *TypeRegistry) Translate(from, to *pbresource.Type) (TranslationFunction, bool) {
	return r.translate(from, to, nil)
}

func (r *TypeRegistry) translate(from, to *pbresource.Type, via []*pbresource.Type) (TranslationFunction, bool) {
	root, ok := r.Resolve(from)
	if !ok {
		return nil, false
	}

	// Simple case. There's a direct mapping (A => B).
	for _, mapping := range root.VersionMappings {
		if EqualType(to, mapping.To) {
			return mapping.Translate, true
		}
	}

	// Complex case, there's an indirect mapping (A => B => C).
outer:
	for _, mapping := range root.VersionMappings {
		// Prevent loops.
		//
		// TODO: something better than a linear scan here!
		for _, v := range via {
			if EqualType(v, mapping.To) {
				continue outer
			}
		}

		translate, ok := r.translate(mapping.To, to, append(via, from))
		if ok {
			return func(src proto.Message) (proto.Message, error) {
				intermediate, err := mapping.Translate(src)
				if err != nil {
					return nil, err
				}
