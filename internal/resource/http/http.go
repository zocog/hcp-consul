...
func NewHandler(client pbresource.ResourceServiceClient, registry resource.Registry) http.Handler {
	...
	for _, t := range registry.Types() {
		// List Endpoint.
		base := fmt.Sprintf("/%s/%s/%s", t.Type.Group, t.Type.GroupVersion, t.Type.Kind)
		mux.Handle(base, http.StripPrefix(base, &listHandler{t, client}))
...
func (h *listHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	rsp, err := h.client.List(r.Context(), &pbresource.ListRequest{
		Type:       h.reg.Type,
		Tenancy:    tenancy(r),
...
func (h *resourceHandler) handleDelete(w http.ResponseWriter, r *http.Request) {
	_, err := h.client.Delete(r.Context(), &pbresource.DeleteRequest{
		Id: &pbresource.ID{
			Type:    h.reg.Type,
			Tenancy: tenancy(r),
			Name:    r.URL.Path,
