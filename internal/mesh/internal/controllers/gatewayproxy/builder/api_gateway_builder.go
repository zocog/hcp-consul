...
func makeInboundListener(name string, port uint32, workload *pbcatalog.Workload) *pbproxystate.Listener {
...
func (b *apiGWProxyStateTemplateBuilder) listenersAndRoutes() ([]*pbproxystate.Listener, map[string]*pbproxystate.Route) {
	...
	for listenerName, listenerConfig := range b.computed.ListenerConfigs {
		listener := makeInboundListener(listenerName, listenerConfig.Port, b.workload.Data)

		for hostname, computedHostname := range listenerConfig.HostnameConfigs {
			computedRoute := computedHostname.Routes

			if computedHostname.Tls == nil {
				router := b.routerForHostname(nil, nil, hostname, computedRoute)

				if router != nil {
					listener.Routers = append(listener.Routers, router)
				}
			} else {
				// We require one certificate per hostname max, this should be separated out
				// based on the SANs in the certificate as well as the ordering in the listener
				// the result should roughly be that if more than a single certificate is specified
				// we multiplex based on SNI with the first certificate being the default certificate
				// for any fallthrough hostnames unless there's a 1-1 with the listener configured hostname
				// on any other certificate. All other secondary certificates only match on SNI specificity
				// if not matched by a higher priority certificate. All of this should happen during
				// the computed state reconciliation, so here we should have only a single certificate.
				if len(computedHostname.Tls.Certificates) != 1 {
					panic("invalid from certificate length validation in calculated configuration")
				}

				router := b.routerForHostname(computedHostname.Tls.TlsParameters, computedHostname.Tls.Certificates[0], hostname, computedRoute)
