...
func createServiceAndDataplane(t *testing.T, node libcluster.Agent, cluster *libcluster.Cluster, proxyID, serviceName string, httpPort, grpcPort int, serviceBindPorts []int) (libservice.Service, error) {
...
func createServerResources(t *testing.T, resourceClient *rtest.Client, cluster *libcluster.Cluster, node libcluster.Agent) (*pbresource.Resource, libservice.Service) {
...
func createClientResources(t *testing.T, resourceClient *rtest.Client, cluster *libcluster.Cluster, staticServerRef *pbresource.Resource, node libcluster.Agent, idx int) libservice.Service {
...
func assertDataplaneContainerState(t *testing.T, dataplane libservice.Service, state string) {
...
func httpRequestToVirtualAddress(clientService libservice.Service) (string, error) {
...
func echoToVirtualAddress(clientService libservice.Service) (string, error) {
	// Test that we can make a request to the virtual ip to reach the upstream.
	//
	// NOTE(pglass): This uses a workaround for DNS because I had trouble modifying
	// /etc/resolv.conf. There is a --dns option to docker run, but it
	...
	if err != nil {
		return out, fmt.Errorf("nc request to upstream virtual address %s:%d\nerr = %v\nout = %s\nservice=%s", staticServerVIP, echoPort, err, out, clientService.GetServiceName())
	}
...
func assertPassing(t require.TestingT, fn func(libservice.Service) (string, error), svc libservice.Service, success bool) {
	_, err := fn(svc)
	if success {
		require.NoError(t, err)
	} else {
