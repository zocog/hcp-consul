...
- `global` ((#v-global)) - Holds values that affect multiple components of the chart.
  ...
  - `domain` ((#v-global-domain)) (`string: consul`) - The domain Consul will answer DNS queries for
    (Refer to [`-domain`](/consul/docsreference-cli/agent#_domain)) and the domain services synced from
  ...
  - `secretsBackend` ((#v-global-secretsbackend)) - secretsBackend is used to configure Vault as the secrets backend for the Consul on Kubernetes installation.
    ...
    Refer to [Vault as the Secrets Backend](/consul/docs/deploy/server/k8s/multi/vault)
  ...
  - `gossipEncryption` ((#v-global-gossipencryption)) - Configures Consul's gossip encryption key.
    (Refer to [`-encrypt`](/consul/docsreference-cli/agent#_encrypt)).
  ...
  - `tls` ((#v-global-tls)) - Enables [TLS](/consul/docs/secure-consul/encryption/tls/builtin)
    ...
    - `enabled` ((#v-global-tls-enabled)) (`boolean: false`) - If true, the Helm chart will enable TLS for Consul
      ...
      This setting is required for [Cluster Peering](/consul/docs/link/cluster-peering/k8s).
    ...
    - `verify` ((#v-global-tls-verify)) (`boolean: true`) - If true, `verify_outgoing`, `verify_server_hostname`,
      ...
      Please refer to [TLS on existing clusters](/consul/docs/secure-mesh/certificate/existing)
      for more details.
  ...
  - `federation` ((#v-global-federation)) - Configure federation.
    ...
    - `k8sAuthMethodHost` ((#v-global-federation-k8sauthmethodhost)) (`string: null`) - If you are setting `global.federation.enabled` to true and are in a secondary datacenter,
      ...
      Please refer to the [Kubernetes Auth Method documentation](/consul/docs/secure-consul/acl/auth-methods/kubernetes).
...
- `server` ((#v-server)) - Server, when enabled, configures a server cluster to run. This should
  ...
  - `storageClass` ((#v-server-storageclass)) (`string: null`) - The StorageClass to use for the servers' StatefulSet storage. It must be
    ...
    Refer to the [Read/Write Tuning](/consul/docs/deploy/server/vm/requirements#read-write-tuning)
    ...
    The [Reference Architecture](/consul/tutorials/production-deploy/reference-architecture#hardware-sizing-for-consul-servers) contains best practices and recommendations for selecting suitable hardware sizes for your Consul servers.
  ...
  - `extraConfig` ((#v-server-extraconfig)) (`string: {}`) - A raw string of extra [JSON configuration](/consul/docs/reference/agent) for Consul
...
- `client` ((#v-client)) - Values that configure running a Consul client on Kubernetes nodes.
  ...
  - `join` ((#v-client-join)) (`array<string>: null`) - A list of valid [`-retry-join` values](/consul/docsreference-cli/agent#_retry_join).
  ...
  - `nodeMeta` ((#v-client-nodemeta)) - nodeMeta specifies an arbitrary metadata key/value pair to associate with the node
    (refer to [`-node-meta`](/consul/docsreference-cli/agent#_node_meta))
  ...
  - `extraConfig` ((#v-client-extraconfig)) (`string: {}`) - A raw string of extra [JSON configuration](/consul/docs/reference/agent) for Consul
...
- `ui` ((#v-ui)) - Values that configure the Consul UI.
  ...
  - `metrics` ((#v-ui-metrics)) - Configurations for displaying metrics in the UI.
    ...
    - `provider` ((#v-ui-metrics-provider)) (`string: prometheus`) - Provider for metrics. Refer to
      [`metrics_provider`](/consul/docs/reference/agent#ui_config_metrics_provider)
  ...
  - `dashboardURLTemplates` ((#v-ui-dashboardurltemplates)) - Corresponds to [`dashboard_url_templates`](/consul/docs/reference/agent#ui_config_dashboard_url_templates)
    configuration.

    - `service` ((#v-ui-dashboardurltemplates-service)) (`string: ""`) - Sets [`dashboardURLTemplates.service`](/consul/docs/reference/agent#ui_config_dashboard_url_templates_service).
...
- `connectInject` ((#v-connectinject)) - Configures the automatic Connect sidecar injector.
  ...
  - `aclBindingRuleSelector` ((#v-connectinject-aclbindingruleselector)) (`string: serviceaccount.name!=default`) - Query that defines which Service Accounts
    ...
    Refer to Auth methods [Binding rules](/consul/docs/secure-consul/acl/auth-methods#binding-rules)
    and [Trusted identiy attributes](/consul/docs/secure-consul/acl/auth-methods/kubernetes#trusted-identity-attributes)
    for more details.
...
- `meshGateway` ((#v-meshgateway)) - [Mesh Gateways](/consul/docs/link/mesh-gateway/) enable Consul Connect to work across Consul datacenters.

  - `enabled` ((#v-meshgateway-enabled)) (`boolean: false`) - If [mesh gateways](/consul/docs/link/mesh-gateway/) are enabled, a Deployment will be created that runs
...
- `apiGateway` ((#v-apigateway)) - [DEPRECATED] Use connectInject.apiGateway instead.
  ...
  - `image` ((#v-apigateway-image)) (`string: null`) - Image to use for the api-gateway-controller pods and gateway instances
    ...
    Using API Gateway <= 0.4 with external servers requires setting `client.enabled: true`.
