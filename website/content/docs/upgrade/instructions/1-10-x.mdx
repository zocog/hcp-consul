...
We do not recommend running in production with ACLs disabled. We recommend enabling ACLs then proceeding with the instructions within the **ACLs Enabled** tab. Refer to [Bootstrap ACL system](/consul/docs/secure-consul/acl/bootstrap) for more information.
