...
Templated policies are a configuration block that you can add to token and role configurations. You can use them during the authorization process to automatically generate a policy for specific use-cases. Consul links the generated policy to the role or token, which provides the required access to a particular resource.
